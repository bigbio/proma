...
function insertSDRFInRecordSheet(modification,parameter,ontologyObject){
    var previousSheet = SpreadsheetApp.getActiveSpreadsheet().getActiveSheet();
    var activeRange = previousSheet.getActiveRange();
    var RecordSheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Record");

    if(RecordSheet == undefined) {
        RecordSheet = SpreadsheetApp.getActiveSpreadsheet().insertSheet("Record");
        RecordSheet.getRange(1, 1).setValue("Parameter Type");
        RecordSheet.getRange(1, 2).setValue("Paremeter Value")
        RecordSheet.getRange(1, 3).setValue("Term Name");
        RecordSheet.getRange(1, 4).setValue("Ontology Source");
        RecordSheet.getRange(1, 5).setValue("Term URI");
        SpreadsheetApp.setActiveSheet(previousSheet)
        SpreadsheetApp.getActiveSheet().setActiveRange(activeRange);
    }
    ...
    if(RecordSheet != undefined) {
        // we have a term sheet, so we can enter information about the term here.
        var insertionRow = findNextBlankRow(RecordSheet);
        if(parameter == 'modification parameters'){
          RecordSheet.getRange(insertionRow, 1).setValue("modification parameters");
        }
        else if(parameter == 'instrument'){
          RecordSheet.getRange(insertionRow, 1).setValue("instrument");
        }
        else if(parameter == 'cleavage agent details'){
          RecordSheet.getRange(insertionRow, 1).setValue("cleavage agent details");
        }
        else if(parameter == 'label'){
          RecordSheet.getRange(insertionRow, 1).setValue("label");
        }
        RecordSheet.getRange(insertionRow, 2).setValue(modification);
        RecordSheet.getRange(insertionRow, 3).setValue(ontologyObject.term);
        RecordSheet.getRange(insertionRow, 4).setValue(ontologyObject.ontologyId);
        RecordSheet.getRange(insertionRow, 5).setValue(ontologyObject.url);
    }
}
